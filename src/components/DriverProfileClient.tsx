...
import { createClient } from "@/utils/supabase/client" // <--- The new client
...
export default function DriverProfileClient({ initialDriver, id }: { initialDriver: DriverRow; id: string }) {
  ...
  const [driver] = useState<DriverRow>(initialDriver) // Data passed from server
  ...
  // Fetch only Private Data & Session on client
  useEffect(() => {
    let mounted = true
    ...
    async function loadPrivate() {
      ...
      if (mounted) setSessionEmail(sessionData.session?.user.email ?? null)

      // Try to get private data (will be null if not unlocked due to RLS)
      ...
      if (mounted) setPriv(privateData ?? null)

      // If redirected from payment success
      if (new URLSearchParams(window.location.search).get("paid") === "1") {
        setTimeout(async () => {
          const { data: privateAgain } = await supabase
            ...
            .eq("driver_id", id)
            .maybeSingle()
          if (mounted) setPriv(privateAgain ?? null)
        }, 1500)
      }
    }
  ...
  return (
    <>
      {/* Header (Inside Client to access session state) */}
...
function StatPill({ title, value, icon }: { title: string; value: string; icon: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 p-3 rounded-2xl bg-white/5 border border-white/10 shadow-sm shadow-black/30">
